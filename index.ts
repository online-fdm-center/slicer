...
import Slic3r from './slic3r'

const check = () => {
    request({
        url: 'http://api.online.fdm.center:3000/getFileToProcess',
        method: 'POST',
        headers: {
        ...
        if (answer === null){
            ...
            try {
                ...
                slic3r.on('done', data => {
                    ...
                    request({
                        url: `http://api.online.fdm.center:3000/files/${answer.id}/setAmount`,
                        method: 'POST',
                        headers: {
                            'x-server-token': process.env.ADMIN_AUTH_TOKEN,
